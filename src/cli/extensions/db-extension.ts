...
async function runTypeORMCommand(command: string, toolbox: Toolbox, additionalParams = '') {
  ...
  const ormConfigFullPath = process.env.WARTHOG_ORMCONFIG_PATH
    ? process.env.WARTHOG_ORMCONFIG_PATH
    : path.join(String(process.env.WARTHOG_GENERATED_FOLDER), 'ormconfig.ts');
